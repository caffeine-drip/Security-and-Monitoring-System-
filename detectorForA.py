...
cmd="DROP TABLE IF EXISTS PresentInSec4A"
cursor=conn.execute(cmd)
conn.commit()
conn.close()

conn=sqlite3.connect("DB/datab2.db")
cmd="CREATE TABLE IF NOT EXISTS PresentInSec4A(Name string,RollNo int PRIMARY KEY)"
cursor=conn.execute(cmd)
conn.commit()
conn.close()
...
def sendData(User):
    ...
    if(sec!="a" and sec !="A"):
        return
    conn=sqlite3.connect("DB/datab2.db")
    cmd="SELECT * FROM PresentInSec4A WHERE RollNo="+str(Rno)
    cursor=conn.execute(cmd)
    ifRecordExist=0
    for row in cursor:
        ifRecordExist=1
    ...
    if(ifRecordExist!=1):
        cmd="INSERT INTO PresentInSec4A(Name,RollNo) Values("+str(name)+","+str(Rno)+")"
        conn.execute(cmd)
        #print"inInsert"
    #else:
...
#faces = faceCascade.detectMultiScale(gray, 1.3, 5)
    ...
    for(x,y,w,h) in faces:
        Idd, conf = recognizer.predict(gray[y:y+h,x:x+w])
