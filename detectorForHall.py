...
cmd="DROP TABLE IF EXISTS PresentInHall"
cursor=conn.execute(cmd)
conn.commit()
conn.close()

conn=sqlite3.connect("DB/datab2.db")
cmd="CREATE TABLE IF NOT EXISTS PresentInHall(Name string,RollNo int PRIMARY KEY)"
cursor=conn.execute(cmd)
conn.commit()
conn.close()
...
def sendData(User):
    ...
    year=User[3]
    conn=sqlite3.connect("DB/datab2.db")
    cmd="SELECT * FROM PresentInHall WHERE Name="+str(name)
    cursor=conn.execute(cmd)
    ifRecordExist=0
    for row in cursor:
        ifRecordExist=1
    ...
    if(ifRecordExist!=1):
        cmd="INSERT INTO PresentInHall(Name,RollNo) Values("+str(name)+","+str(Rno)+")"
        conn.execute(cmd)
        #print"inInsert"
    #else:
